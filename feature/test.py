...
def dictionary_and_embedding(ast, size=128):
    # self.w2v_path = 'subtrees/'+args.input+'/node_w2v_'+str(size)
    # if not os.path.exists('subtrees/'+args.input):
    #     os.mkdir('subtrees/'+args.input)
    from prepare_data import get_sequences
...
def dictionary_and_embedding_debug(ast, size=128):
    from prepare_data import get_sequences
    sequence = []
    get_sequences(ast, sequence)
...
if __name__ == '__main__':
    ast = parse_ast('void main() { int a = 0; }')  # Example usage of parse_ast
    print(ast)

    dictionary_and_embedding_debug(ast)
