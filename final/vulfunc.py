...
def prepare_data_multiclass(token_ids_data, labels, vocab, max_len=128, test_size=0.2, batch_size=32):
    """
    Prepare data for training by padding sequences, splitting into train/test sets,
    and creating DataLoader objects.
    ...
    Args:
        token_ids_data: List of lists containing token IDs
        labels: List of labels (cwe categories)
        max_len: Maximum sequence length for padding
        test_size: Proportion of data to use for testing
        batch_size: Batch size for training
...
def generate_vocab():
    ...
    # Load the dataset
    sard_dataset   = load_json_file(DATASET_PATH[0])
    bigvul_dataset = load_json_file(DATASET_PATH[1])
    ...
    # Get code columns
    df1_code = sard_dataset[['code']].copy()
    df2_code = bigvul_dataset[['func_before']].copy()
    df3_code = devign_dataset[['code']].copy()

    # Rename columns to 'code' for consistency
    df3_code.rename(columns={'func_before': 'code'}, inplace=True)

    df1_target = sard_dataset[['target']].copy()
    df2_target = bigvul_dataset[['target']].copy()
    df3_target = devign_dataset[['target']].copy()
    
    # Concatenate the DataFrames
    dataset = pd.concat([df1_code, df2_code, df3_code], ignore_index=True)
    # labels = pd.concat([df1_target, df2_target, df3_target], ignore_index=True)
    ...
    with open('CXX_AST_DATA/token_vectors_final.txt', 'w', encoding='utf-8') as f:
    ...
    # labels.to_json('CXX_AST_DATA/bigvul_labels.json', orient='records', lines=False, indent=4)
    # with open('CXX_AST_DATA/labels.json', 'w', encoding='utf-8') as f:
    #     json.dump(, f, indent=4, ensure_ascii=False)

    # Save the vocabulary to files
    vocab_mgr.save_vocab(index_filepath='vocab_final_index.json',
                         freq_filepath='vocab_freq_final.json')
    logger.info("[*] Vocabulary saved to vocab.json")

def train_multiclass(dataset_name="sard", model="lstm", vocab_path='CXX_AST_DATA/vocab/vocab_final_index.json'):
    ...
    if dataset_name == "sard":
        ...
        # Filter the DataFrame to include only rows with these CWE values
        train = train[train['cwd'].isin(top_cwes)]

        # Balance the dataset by downsampling to cwe with least samples
        train = train.groupby('cwd').apply(lambda x: x.sample(n=train['cwd'].value_counts().min())).reset_index(drop=True)

        # Summarize the dataset
        logger.info(f"Dataset size after filtering: {len(train)}")
        print(f"Dataset size after filtering: {len(train)}")
        top_cwes = train['cwd'].value_counts()
        print(top_cwes)
        ...
        labels = train['cwd'].map(class_to_idx)
    elif dataset_name == "bigvul":
        # Print top 5 CWE categories (by cwd_id + target = 1)
        print("Top 10 CWE categories in BigVul dataset:")
        # Keep target = 1
        train = train[train['target'] == '1']
        # print(train)
        top_cwes = train['cwd_id'].value_counts().head(10)
        print(top_cwes)
        return
...
def parse_options():
    ...
    parser.add_argument('-d', '--dataset', default='sard', choices=['sard', 'bigvul', 'devign'],)
    parser.add_argument('-mc', '--multi-class', action='store_true',
                        help='Train multi-class model', required=False)
...
class VulFuncPipeline:
    ...
    def run(self, args):
        ...
        if args.train:
            if args.multi_class:
                logger.info("[+] Training multi-class model...")
                train_multiclass(dataset_name=args.dataset, model=args.model, vocab_path=VOCAB_PATH)
            else:
                logger.info("[+] Training the model...")
                train(model=args.model, dataset_name=args.dataset, vocab_path=VOCAB_PATH)
