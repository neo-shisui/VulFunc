...
# Training function
def train_rnn(model, train_loader, test_loader, criterion, optimizer, num_epochs, device, num_classes=2):
    ...
    for epoch in range(num_epochs):
        ...
        # Print predict output and labels for debugging
        print(f'Epoch {epoch+1}/{num_epochs}, Train Loss: {epoch_loss:.4f}')
        print("Raw logits:", outputs[:5])  # Show first 5 outputs
        print("Predicted classes:", torch.max(outputs, 1)[1][:5])  # Show predicted classes
        print("True labels:", labels[:5])  # Show true labels
