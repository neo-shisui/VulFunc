...
# Training function
def train_lstm(model, train_loader, test_loader, criterion, optimizer, num_epochs, device, num_classes=2, patience=5):
    train_losses = []
    test_losses = []
    test_accuracies = []
    test_precisions = []
    ...
    for epoch in range(num_epochs):
        ...
        epsilons = 1e-6
        if test_loss < best_loss - epsilons:
            best_loss = test_loss
            epochs_no_improve = 0
        else:
            epochs_no_improve += 1
...
def train_bilstm(model, train_loader, test_loader, optimizer, scheduler, epochs, device, criterion, patience=5, num_classes=2):
